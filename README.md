  ...
  "posts": [
    {
      ...
      "views": 648000,
      "reactions": [ { "emoji": "🤡", "count": 283 }, { "emoji": "👍", "count": 38 } ]
    }
  ]
}
```
...
- `reactions` — массив `{emoji, count}`; поле опускается, если реакций нет. Платные (звёзды) → `emoji: "paid"`.
- `channel.subscribers_raw` — строка как на странице; `subscribers` — распарсенное число (или `null`).
- Лимит — последние 30 постов.

> **Чего публичная `t.me/s/` НЕ отдаёт:** счётчики комментариев и пересылок/«поделились». Они доступны только через Telegram Bot API / MTProto (бот-участник канала) — это вне данного маршрута.
